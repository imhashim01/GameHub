...
function sendRequest(PDO $db, array $body): void {
    ...
    if (!$senderId) {
        jsonResponse(['success' => false, 'message' => 'Login required to send requests.', 'auth' => false]);
    }
    if (!$receiverId) {
    ...
    $check = $db->prepare('SELECT id, username FROM users WHERE id = ? LIMIT 1');
    ...
    if ($row) {
        $statusMap = [
            'pending'  => 'A request is already pending with this user.',
            'accepted' => 'You are already partners with this user! 🎮',
            'rejected' => 'This request was previously rejected.',
        ];
        jsonResponse(['success' => false, 'message' => $statusMap[$row['status']] ?? 'Request already exists.']);
    }
    ...
    jsonResponse([
        'success'  => true,
        'message'  => "Request sent to {$receiver['username']}! 🎮",
...
function respondRequest(PDO $db, array $body, string $status): void {
    ...
    if (!$requestId) jsonResponse(['success' => false, 'message' => 'Request ID required.'], 400);

    // Only the RECEIVER can accept/reject
    $stmt = $db->prepare(
        'SELECT pr.*, u.username AS sender_name
    ...
    $msg = $status === 'accepted'
        ? "You are now partners with {$request['sender_name']}! 🎮 Go play together!"
        : "Request from {$request['sender_name']} rejected.";
...
// ── List received requests (for notifications) ────────────────
function listReceived(PDO $db): void {
    $userId = currentUserId();
    if (!$userId) jsonResponse(['success' => false, 'auth' => false, 'requests' => []]);

    $stmt = $db->prepare(
        'SELECT pr.id, pr.message, pr.status, pr.created_at,
                u.id   AS sender_id,
...
// ── Pending count (for badge on nav) ─────────────────────────
function pendingCount(PDO $db): void {
    $userId = currentUserId();
    if (!$userId) jsonResponse(['success' => true, 'count' => 0]);

    $stmt = $db->prepare(
        'SELECT COUNT(*) FROM partner_requests WHERE receiver_id = ? AND status = "pending"'
    );
    $stmt->execute([$userId]);
